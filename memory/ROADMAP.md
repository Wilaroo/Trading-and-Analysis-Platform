...
- **UI panel `shares` vs `remaining_shares` confusion**: the OPEN(11) / Manage Open Positions panel currently displays `shares` as the position size, which masks zombies (rs=0 trade with shares=256 looks like a 256-sh position). Fix V6 panel rendering (or V5 patch) to show `remaining_shares` as the live count, with `shares` shown only as the original allocation in expanded row.
