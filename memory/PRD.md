...
- `dynamic_universe_builder.py` composes a priority-ranked daily scan universe
  (liquid core + IB movers + catalysts + held, regime-tilted), gated to the
  qualified universe, persisted to `daily_scan_universe`, rebuilt premarket +
  every ~45 min intraday. Graceful degradation never yields empty.
- Wired: front-loaded into daily/premarket scans, injected into RTH wave_scanner
  Tier-1 (every ~15s), top findings pushed to the game plan (`dynamic_movers`,
  UTC-date keyed). API: `/api/dynamic-universe` (GET/priority/POST rebuild).
- Frontend: Game Plan tab "Dynamic Movers" panel (regime chip + Refresh +
  colour-coded source tags). yarn build done, backend restarted, health green.
- Live DGX rebuild confirmed real data (NVDA/AAPL 105, AIQ held 100, VSCO
  earnings 60, PANW 49...). Tests 6/6.
...
- `WS /api/ws/stream` — **v19.34.184 (2026-05-30)** — Mission Control live pipeline bus. Fans out `emit_stream_event` output, classified into 5 lanes (scanner/gates/execution/position/reconciler) + system, to connected clients. Client subscribes with `{action:"subscribe", lanes[], severities[], mode:"aggregate|raw"}`. Backed by `services/stream_bus.StreamBus` (sync `publish()` on the hot path, ~300ms batch flush, scanner-skip firehose aggregated into a periodic `scan_pulse`, zero overhead when no client connected). Persistence to `sentcom_thoughts` is always-on; this WS is delivery-only. Frontend: new top-level **Mission Control** tab (`pages/MissionControlPage.jsx`), backfills via `GET /api/sentcom/stream/history` then tails live. **v19.34.188 (2026-05-30)** added lifecycle emits — `order_submitted` / `partial_fill` (Execution lane), `stop_to_breakeven` / `trailing_stop_moved` (Position lane) — and an inline **"Ack + Unlock"** action on System/Safety alarm rows (`components/missioncontrol/SafetyRow.jsx` → `POST /api/safety/reset-kill-switch`) plus local dismiss.
...
- `trading_bot_service._get_trade_alerts` (`_alert_rank`) + `eod_generation_service` — **v19.34.185 / F-F (2026-05-30)** — gameplan-aware prioritization. A 09:00 ET weekday scheduler force-regenerates today's `game_plan` premarket (capturing `premarket_scanner`/`daily_scanner` conviction names + market bias, stored stably). The slot ranker then applies a mild, env-tunable, ranking-only conviction boost: `+GAMEPLAN_WATCHLIST_BOOST` (default 8) for gameplan-watchlist symbols and `+GAMEPLAN_BIAS_BOOST` (default 4) for bias-aligned direction. Never changes TQS grade or gate decisions; priority bucket still dominates. Helpers: `_compute_gameplan_boost` (static), `_get_gameplan_conviction` (premarket/daily-sourced only, ~5min cache). Set both boosts to 0 to disable.
