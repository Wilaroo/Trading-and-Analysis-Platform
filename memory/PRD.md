...
- `services/state_integrity_service.py` + `GET /api/trading-bot/integrity-status` + `POST /api/trading-bot/force-resync` — **v19.34.10 (2026-05-06)** — drift watchdog catching the v19.34.9 class of bug (in-memory `risk_params` vs persisted `bot_state.risk_params` divergence). 60s background loop with per-field policy: **Mongo wins** for capital/limit fields (`starting_capital`, `max_daily_loss*`, `max_open_positions`, `max_position_pct`, `min_risk_reward`, `max_notional_per_trade`, `max_risk_per_trade`, `reconciled_default_*`); **memory wins** for `setup_min_rr` (operator hot-tuned). Auto-resolve ON by default; flip via `STATE_INTEGRITY_AUTO_RESOLVE=false` for detect-only. Drift events persist to `state_integrity_events` (TTL 7d) + emit CRITICAL `state_drift_detected_v19_34_10` Unified Stream event. Feature flags: `STATE_INTEGRITY_CHECK_ENABLED=true`, `STATE_INTEGRITY_CHECK_INTERVAL_S=60` (5s floor).
