...
- `GET /api/assistant/coach/morning-briefing` — Setup-landscape-grounded morning briefing in 1st-person voice (2026-04-29 v3)
...
- `GET /api/scanner/setup-landscape?context=morning|midday|eod|weekend` — universe-wide Bellafiore-Setup snapshot + 1st-person narrative (2026-04-29 v3)
...
### Runtime architecture (decided 2026-04-29 evening)
...
**Everything else flows in as features** to the per-Trade ML models:
  - Multi-index regime tag (planned next session — `MultiIndexRegimeClassifier`)
...
- `backend/services/enhanced_scanner.py` — 38 trade detectors, scanner loop, `_apply_setup_context` matrix gate, `LiveAlert` dataclass with new context fields (`market_setup`, `is_countertrend`, `out_of_context_warning`, `experimental`)
- `backend/services/market_setup_classifier.py` — `MarketSetup` enum (7 + NEUTRAL), `MarketSetupClassifier` (daily-bar driven), `TRADE_SETUP_MATRIX`, `TRADE_ALIASES`, `EXPERIMENTAL_TRADES`, `lookup_trade_context()`
- `backend/services/setup_landscape_service.py` — `SetupLandscapeService` with 4 narrative voices (morning/midday/eod/weekend), 1st-person voice rules, 60s cache
- `backend/services/ai_assistant_service.py` — `get_coaching_alert()` injects landscape narrative + voice rules into AI prompts
