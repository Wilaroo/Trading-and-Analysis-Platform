...
- `GET /api/system/banner` — **v19.30.11 (2026-05-01)** — operator-facing alert tile. Returns `{level, message, detail, action, since_seconds, subsystem, as_of}`. Drives the giant red SystemBanner.jsx strip when pusher_rpc red ≥30s or mongo red ≥10s. Action text explicitly tells operator NOT to restart Spark backend when pusher is the problem.
