...
|   |   +-- SystemStatusContext.jsx   # Fixed: reduced startup delay from 20s to 3s
|   |   +-- ConnectionManagerContext.jsx # Fixed: reduced startup delay from 20s to 3s
|   +-- hooks/
|   |   +-- useVisibility.js
|   |   +-- useSmartPolling.js
...
**Fixes applied**:
1. **Alpaca Service** (`services/alpaca_service.py`): Wrapped all sync SDK calls in `asyncio.to_thread()`
2. **Technical Service** (`services/realtime_technical_service.py`): Wrapped sync pymongo call in `asyncio.to_thread()`
...
**Result**: All endpoints now respond in <2ms locally (was 6-47s)

### StartupModal Fix (March 24, 2026) - COMPLETED
**Fixes applied**:
1. Created `/api/startup-check` endpoint that returns ALL service statuses from in-memory state
2. AI status check now verifies ANY LLM provider (Emergent OR Ollama), not just Ollama proxy
3. Rewrote `StartupModal.jsx` to use single consolidated endpoint
4. Made `startup_event` non-blocking by deferring heavy ops into `asyncio.create_task()`

### Data Loading Delay Fix (March 24, 2026) - COMPLETED
**Root cause**: `SystemStatusContext` and `ConnectionManagerContext` had 20-second delays before starting health checks. These were added to avoid request flooding during the old slow startup modal. With the new fast modal, these delays blocked data loading for 17+ seconds after app was ready.

**Fix**: Reduced delays from 20s to 3s in both contexts.

### Fill-Gaps Endpoint Fix (March 24, 2026) - COMPLETED
**Fix**: DB operations wrapped in `asyncio.to_thread()`
**Result**: Returns in ~25 seconds for 442 symbols (was hanging indefinitely)
...
- `GET /api/startup-check` - Consolidated startup status (~2ms, in-memory only)
- `POST /api/ib-collector/fill-gaps` - Smart gap filler (~25s)
- `GET /api/dashboard/init` - Batch dashboard init
- `GET /api/system/monitor` - System health monitor
