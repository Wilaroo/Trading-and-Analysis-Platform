...
:: =====================================================
:: STEP 1/9: CHECK SPARK NETWORK CONNECTION
:: =====================================================
...
:: =====================================================
:: STEP 2/9: GIT PULL (Both Windows and Spark)
:: =====================================================
...
ssh -n -o BatchMode=yes %SPARK_USER%@%SPARK_IP% "cd %SPARK_REPO% && git pull origin main"
if not %errorlevel%==0 (
    echo.
    echo ============================================
    echo [ABORT] SSH git pull to Spark FAILED.
    echo ============================================
    echo   Running services with stale code is dangerous
    echo   after recent validator/MC/backtest fixes.
    echo.
    echo   Fix options:
    echo     1. Verify SSH keys are set up ^(see README^)
    echo     2. Pull manually on Spark:
    echo        ssh %SPARK_USER%@%SPARK_IP%
    echo        cd %SPARK_REPO% ^&^& git pull
...
:: =====================================================
:: STEP 2.6/9: VERIFY CRITICAL FIX MARKERS ARE PRESENT
:: =====================================================
echo [2.6/9] Verifying critical code fixes are present on Spark...
ssh -n -o BatchMode=yes %SPARK_USER%@%SPARK_IP% ^
    "cd %SPARK_REPO% && grep -q 'FAIL-CLOSED' backend/services/ai_modules/post_training_validator.py && grep -q 'bootstrap: bool = True' backend/services/slow_learning/advanced_backtest_engine.py && grep -q 'required_ai_direction' backend/services/slow_learning/advanced_backtest_engine.py && grep -q 'regime_diversity' backend/services/ai_modules/vae_regime.py && echo VERIFIED" > "%TEMP%\fix_check.tmp" 2>nul
findstr /C:"VERIFIED" "%TEMP%\fix_check.tmp" >nul 2>&1
if not %errorlevel%==0 (
    echo.
    echo ============================================
    echo [ABORT] Critical code fix markers MISSING on Spark.
    echo ============================================
    echo   Expected after recent session:
    echo     - FAIL-CLOSED      (post_training_validator.py)
    echo     - bootstrap        (advanced_backtest_engine.py)
    echo     - required_ai_direction  (short-side fix)
    echo     - regime_diversity (vae_regime.py)
    echo.
    echo   Cause: Emergent hasn't pushed latest commits to GitHub.
    echo   Fix:   Click "Save to Github" in Emergent chat, then re-run.
    echo ============================================
    del "%TEMP%\fix_check.tmp" 2>nul
    pause
    exit /b 1
)
del "%TEMP%\fix_check.tmp" 2>nul
echo        All critical fix markers verified!
echo.

:: =====================================================
:: STEP 2.7/9: STOP EXISTING SPARK SERVICES (clean restart)
:: =====================================================
echo [2.7/9] Stopping existing Spark services (via spark_stop.sh)...
ssh -n -o BatchMode=yes %SPARK_USER%@%SPARK_IP% "cd %SPARK_REPO% && bash scripts/spark_stop.sh" 2>nul
echo        Spark cleanup complete.
echo.

:: =====================================================
:: STEP 3/9: START SPARK SERVICES (fresh)
:: =====================================================
echo [3/9] Starting DGX Spark services (via spark_start.sh)...
ssh -n -o BatchMode=yes %SPARK_USER%@%SPARK_IP% "cd %SPARK_REPO% && bash scripts/spark_start.sh" 2>nul
...
:: =====================================================
:: STEP 4/9: IB GATEWAY LOGIN (Windows)
:: =====================================================
...
:: =====================================================
:: STEP 5/9: START IB DATA PUSHER (Windows -> Spark)
:: =====================================================
echo [5/9] Starting IB Data Pusher (live market data)...
...
:: =====================================================
:: STEP 6/9: START TURBO COLLECTORS (idle until NIA triggers)
:: =====================================================
echo [6/9] Starting %NUM_COLLECTORS% Turbo Collectors (idle until collection triggered)...
...
:: =====================================================
:: STEP 7/9: OPEN BROWSER + TRAINING MONITOR
:: =====================================================
...
start "[SPARK MONITOR] Training Pipeline" ssh -t %SPARK_USER%@%SPARK_IP% "cd %SPARK_REPO% && bash documents/scripts/monitor_training.sh"
echo        Training monitor terminal opened (connects via SSH)
echo.
