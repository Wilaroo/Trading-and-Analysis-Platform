...
class EnhancedBackgroundScanner:
            ...
            try:
                ...
                if current_window == TimeWindow.PREMARKET:
                    ...
                    if self._scan_count % 2 == 0 or self._scan_count == 0:
                        ...
                        try:
                            await self._maybe_rebuild_dynamic_universe()
    ...
    async def _maybe_rebuild_dynamic_universe(self):
        """v19.34.211 — trigger a cadence-gated (45-min TTL) rebuild of the
        Dynamic Universe. Cheap no-op when a fresh doc already exists."""
        try:
            from services.dynamic_universe_builder import get_dynamic_universe_builder
            await get_dynamic_universe_builder(self.db).maybe_rebuild()
        except Exception as e:
            logger.debug(f"dynamic-universe rebuild skipped: {e}")

    def _merge_dynamic_priority(self, symbols: List[str]) -> List[str]:
        """v19.34.211 — front-load today's Dynamic Universe priority names
        (movers + catalysts + held) ahead of the rotation wave so they're
        scanned every pass, while the full-universe rotation continues
        underneath. Degrades to the unchanged wave if the builder has no
        fresh doc."""
        try:
            from services.dynamic_universe_builder import get_dynamic_universe_builder
            prio = get_dynamic_universe_builder(self.db).get_priority_symbols()
        except Exception as e:
            logger.debug(f"dynamic-priority merge skipped: {e}")
            return symbols
        if not prio:
            return symbols
        seen = set()
        merged = []
        for s in list(prio) + list(symbols):
            if s and s not in seen:
                seen.add(s)
                merged.append(s)
        return merged

    async def _maybe_auto_execute_daily(self, alert):
    ...
    async def _scan_daily_setups(self):
        ...
        try:
            ...
            try:
                ...
                symbols = self._merge_dynamic_priority(symbols)
            except Exception as e:
    ...
    async def _process_new_alert(self, alert: LiveAlert):
        ...
        # === DMA DIRECTIONAL FILTER (2026-06-22 A7 SOFTENED) ===
        ...
        # v19.34.x (UI Track A / A2h) — also re-enrich when the per-pillar
