        ...
        if self._technical_service and any(v is None for v in [rsi, ma_stack, atr_percent, rvol]):
            try:
                ...
                if snapshot:
                    ...
                    # ma_stack to "neutral" for 100% of alerts.
                    # v19.34.222 — REVERTED the v221 EMA-alignment attempt: the
                    # snapshot's ema_9/ema_20 are INTRADAY while ema_50/sma_200
                    # are DAILY, so any `ema_9>ema_20>ema_50` stack re-introduces
                    # the exact timeframe mix v215 removed (it made ma_stack MORE
                    # neutral, 78%→84%). The intraday `trend` (EMA9/20 + 0.25%
                    # tolerance + v166 macro-veto) is the correct single-timeframe
                    # signal; ~78% neutral is an accurate read of a chopping tape,
                    # not a bug. Reuse it directly.
