            ...
            if ib_pos_map is not None:
                ...
                for _tid, _trade in list(bot._open_trades.items()):
                    try:
                        ...
                        # v402b -- reset OCA zero-share debounce streak the
                        # moment IB reports ANY shares (either side), so a
                        # one-tick transient 0-snapshot can never accumulate.
                        if not (ib_qty_my_dir == 0 and ib_qty_opp_dir == 0):
                            if getattr(_trade, "_oca_zero_streak", 0):
                                try:
                                    _trade._oca_zero_streak = 0
                                except Exception:
                                    pass
                        ...
                        if (
                            ...
                            if _executed_at_e:
                                try:
                                    ...
                                    age_ok_e = True
                            if age_ok_e:
                                # -- v402b (2026-06-23) OCA close DEBOUNCE --
                                # diag_oca_close_transient_vs_reentry.py proved
                                # FOXA/KR/LULU were marked oca_closed_externally
                                # 30-60s after a FRESH fill while the fill tape
                                # showed the position fully held (zero cover
                                # fills) -- a transient IB 0-share snapshot.
                                # Require N CONSECUTIVE zero-both-dir reads
                                # before closing. OCA_CLOSE_ZERO_STREAK default
                                # 2 (set 1 to disable). Streak resets at loop top.
                                import os as _os_deb
                                try:
                                    _need_zero = int(_os_deb.environ.get(
                                        "OCA_CLOSE_ZERO_STREAK", "2"))
                                except (TypeError, ValueError):
                                    _need_zero = 2
                                _zero_streak = int(
                                    getattr(_trade, "_oca_zero_streak", 0) or 0) + 1
                                try:
                                    _trade._oca_zero_streak = _zero_streak
                                except Exception:
                                    pass
                                if _zero_streak < max(1, _need_zero):
                                    logger.info(
                                        "[v402b OCA debounce] %s zero-share "
                                        "read %d/%d -- deferring external-close "
                                        "(transient snapshot guard).",
                                        _trade.symbol, _zero_streak, _need_zero)
                                    continue
