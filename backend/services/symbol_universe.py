...
def get_universe_ranked(
    ...
    *,
    ...
    """Return qualified symbols ordered by ``avg_dollar_volume`` DESC (most
    liquid first), optionally truncated to ``limit``.

    Use this anywhere a *deterministic, non-alphabetical* ordering of the
    universe is needed — e.g. scan-wave rotation or "top-N liquid names".
    ``sorted(get_universe(...))[:N]`` is an ANTI-PATTERN: ``get_universe``
    returns an unordered set, so sorting it is alphabetical and truncating it
    restricts scans to A–early-B names only (the cause of the v19.34.210
    alphabetical trade-selection bias).
    """
    if db is None:
        return []
    tier_key = "investment" if tier == "all" else tier
