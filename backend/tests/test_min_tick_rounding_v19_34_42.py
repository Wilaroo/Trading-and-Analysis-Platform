...
class TestRoundToTick(unittest.TestCase):
    ...
    def test_no_floating_point_artifacts(self):
        ...
        cases = [(12.35, 0.01), (100.123, 0.01), (0.0567, 0.0001),
                 (5.005, 0.01), (10.999, 0.01)]
    ...
    def test_handles_string_min_tick(self):
        from services.ib_direct_service import IBDirectService
        self.assertEqual(IBDirectService._round_to_tick(12.345, "0.01"), 12.35)
...
class TestResolveMinTick(unittest.IsolatedAsyncioTestCase):
    async def test_returns_ib_reported_min_tick(self):
        ...
        row = MagicMock(); row.minTick = 0.0001
        svc._ib = MagicMock()
        svc._ib.reqContractDetailsAsync = AsyncMock(return_value=[row])
        self.assertEqual(await svc._resolve_min_tick(contract), 0.0001)
    ...
    async def test_falls_back_to_penny_on_lookup_failure(self):
        ...
        svc = get_ib_direct_service()
        svc._min_tick_cache = {}
        contract = MagicMock(); contract.symbol = "BOOM"; contract.currency = "USD"
        svc._ib = MagicMock()
        svc._ib.reqContractDetailsAsync = AsyncMock(side_effect=RuntimeError("net error"))
        self.assertEqual(await svc._resolve_min_tick(contract), 0.01)

    async def test_falls_back_to_penny_on_empty_details(self):
        from services.ib_direct_service import get_ib_direct_service
        svc = get_ib_direct_service()
        svc._min_tick_cache = {}
        contract = MagicMock(); contract.symbol = "EMPTY"; contract.currency = "USD"
        svc._ib = MagicMock()
    ...
    async def test_cache_hit_avoids_second_ib_call(self):
        ...
        contract = MagicMock(); contract.symbol = "CACHE"; contract.currency = "USD"
        svc._ib = MagicMock()
        svc._ib.reqContractDetailsAsync = AsyncMock(side_effect=Exception("nope"))
        self.assertEqual(await svc._resolve_min_tick(contract), 0.0001)
    ...
    async def test_zero_min_tick_from_ib_is_promoted_to_penny(self):
        ...
        svc = get_ib_direct_service()
        svc._min_tick_cache = {}
        contract = MagicMock(); contract.symbol = "ZERO"; contract.currency = "USD"
        row = MagicMock(); row.minTick = 0.0
        svc._ib = MagicMock()
        svc._ib.reqContractDetailsAsync = AsyncMock(return_value=[row])
        self.assertEqual(await svc._resolve_min_tick(contract), 0.01)
