...
# ── v19.34.169 log archive ─────────────────────────────────────────
# Auto-archive /tmp/backend.log before any restart so we don't lose
# yesterday's EOD evidence again. Keeps 30 days of history.
_archive_backend_log() {
    local logs_dir="$REPO_DIR/logs"
    mkdir -p "$logs_dir"
    if [ -s /tmp/backend.log ]; then
        local stamp
        stamp="$(date +%Y%m%d_%H%M%S)"
        cp /tmp/backend.log "$logs_dir/backend_${stamp}.log"
        echo "→ archived /tmp/backend.log → logs/backend_${stamp}.log ($(wc -c < /tmp/backend.log) bytes)"
    fi
    # Prune archives older than 30 days
    find "$logs_dir" -name 'backend_*.log' -type f -mtime +30 -delete 2>/dev/null || true
}
# ───────────────────────────────────────────────────────────────────
