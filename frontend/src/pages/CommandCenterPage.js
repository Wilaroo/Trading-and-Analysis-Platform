...
const CommandCenterPage = () => {
  ...
  return (
    <div className="space-y-4 pb-8" data-testid="command-center-page">
      ...
      <div className="space-y-4">
        {/* Top Row - Holdings & Watchlist side by side */}
          ...
          {expandedSections.comprehensiveAlerts && (
            <div className="space-y-3">
              ...
              <div className="grid md:grid-cols-2 gap-3 max-h-[500px] overflow-y-auto">
                {getFilteredComprehensiveAlerts().slice(0, 20).map((alert, idx) => (
                  <motion.div 
                    key={alert.id}
              ...
              {getFilteredComprehensiveAlerts().length > 20 && (
                <p className="text-center text-zinc-500 text-xs py-2">
                  +{getFilteredComprehensiveAlerts().length - 20} more alerts
                </p>
              )}
            </div>
          )}
        ...
        <Card>
          ...
          {expandedSections.breakouts && (
            <div className="space-y-2">
              {!isConnected && (
                <div className="text-center py-4 text-zinc-500 text-sm">
                  Connect IB Gateway for breakout alerts
                </div>
              )}
              ...
              {isConnected && breakoutAlerts.length === 0 && (
                <div className="text-center py-6 text-zinc-500">
                  ...
                  <p className="text-sm">No breakouts detected</p>
                  <p className="text-xs mt-1">Monitoring for stocks breaking key levels...</p>
                </div>
              )}
              
              <div className="grid md:grid-cols-2 gap-2">
                {breakoutAlerts.map((alert, idx) => (
                  <div 
                    key={idx}
                    onClick={() => setSelectedTicker({ symbol: alert.symbol, quote: { price: alert.current_price } })}
                    className={`p-3 rounded-lg cursor-pointer transition-all border ${
                      alert.breakout_type === 'LONG' 
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-white">{alert.symbol}</span>
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${
                          alert.breakout_type === 'LONG' ? 'bg-green-500 text-black' : 'bg-red-500 text-white'
                        }`}>
                          {alert.breakout_type}
                        </span>
                      </div>
                      <span className="text-sm font-bold text-yellow-400">{alert.breakout_score}</span>
                    </div>
                    
                    <div className="grid grid-cols-3 gap-2 text-[9px] mb-2">
                      <div>
                        <span className="text-zinc-500">Price: </span>
                        <span className="text-white font-mono">${alert.current_price}</span>
                      </div>
                      <div>
                        <span className="text-zinc-500">Level: </span>
                        <span className="text-cyan-400 font-mono">${alert.breakout_level}</span>
                      </div>
                      <div>
                        <span className="text-zinc-500">RVOL: </span>
                        <span className="text-purple-400 font-mono">{alert.rvol}x</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-2">
                      <div className="flex items-center justify-between text-[9px] mb-1">
                        <span className="text-zinc-500">Signal: {alert.signal_strength_label}</span>
                        <span className={`font-mono ${
                          alert.rules_matched >= 10 ? 'text-green-400' :
                          alert.rules_matched >= 7 ? 'text-cyan-400' :
                          alert.rules_matched >= 4 ? 'text-yellow-400' :
                          'text-zinc-400'
                        }`}>{alert.rules_matched}/77</span>
                      </div>
                      <div className="h-1 bg-zinc-800 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${
                            alert.signal_strength >= 13 ? 'bg-green-500' :
                            alert.signal_strength >= 9 ? 'bg-cyan-500' :
                            alert.signal_strength >= 5 ? 'bg-yellow-500' :
                            'bg-zinc-600'
                          }`}
                          style={{ width: `${alert.signal_strength}%` }}
                        />
                      </div>
                    </div>
                  </div>
        ...
        <Card>
          ...
          {expandedSections.earnings && (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[400px] overflow-y-auto">
              {earnings.length > 0 ? earnings.map((item, idx) => {
                ...
                const getReactionColor = (reaction) => {
                  if (reaction <= -5) return 'text-red-400';
                  if (reaction < 0) return 'text-orange-400';
                  if (reaction <= 3) return 'text-zinc-400';
                  if (reaction <= 7) return 'text-cyan-400';
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 text-[9px]">
                      <span className="text-zinc-500">
                        Avg: <span className={getReactionColor(avgReaction)}>{avgReaction >= 0 ? '+' : ''}{avgReaction.toFixed(1)}%</span>
                      </span>
                      <span className="text-zinc-500">
                        IV: <span className={ivRank >= 60 ? 'text-yellow-400' : 'text-zinc-300'}>{ivRank.toFixed(0)}%</span>
                      </span>
                    </div>
                    
                    {topStrategy && (
                      <div className="mt-2 p-1.5 rounded bg-cyan-500/10 text-[9px]">
                        <span className="text-cyan-400">{topStrategy.name}</span>
                      </div>
                    )}
                  </div>
                );
              }) : (
                <p className="text-center text-zinc-500 text-sm py-4 col-span-full">No upcoming earnings</p>
              )}
            </div>
          )}
        </Card>

        {/* 7. Short Squeeze Watchlist */}
        <Card>
          <button 
          ...
          {expandedSections.squeeze && (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[300px] overflow-y-auto">
              {shortSqueezeCandidates.length > 0 ? shortSqueezeCandidates.map((stock, idx) => (
                <div 
                  key={idx}
                  onClick={() => setSelectedTicker({ symbol: stock.symbol, quote: { price: stock.price } })}
                  className="p-3 bg-zinc-900/50 rounded-lg cursor-pointer hover:bg-zinc-900 transition-colors border border-white/5 hover:border-red-500/30"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-white">{stock.symbol}</span>
                      <span className={`text-xs px-1.5 py-0.5 rounded font-bold ${
                        stock.squeeze_score >= 80 ? 'bg-red-500 text-white' :
                        stock.squeeze_score >= 60 ? 'bg-orange-500 text-black' :
                ...
                <p className="text-center text-zinc-500 text-sm py-4 col-span-full">No squeeze candidates found</p>
              )}
            </div>
          )}
        </Card>

        {/* Recent System Alerts (collapsed by default) */}
        <Card>
          <button 
          ...
          >
            <div className="flex items-center gap-2">
              <Bell className="w-5 h-5 text-yellow-400" />
              <h3 className="text-sm font-semibold uppercase tracking-wider">System Alerts</h3>
            </div>
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 border-b border-white/10 bg-gradient-to-r from-cyan-900/20 to-transparent">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded ${
                    selectedEnhancedAlert.direction === 'LONG' 
                      ? 'bg-green-500/20 text-green-400' 
                      : 'bg-red-500/20 text-red-400'
                  }`}>
                    {selectedEnhancedAlert.direction}
                  </span>
                </div>
              ...
              <div className="p-4 overflow-y-auto max-h-[65vh] space-y-4">
                ...
                <div className="flex items-center gap-3 text-sm">
                  <div className="flex items-center gap-1.5 text-cyan-400">
                    ...
                    {selectedEnhancedAlert.triggered_at_formatted || 'Just now'}
                  </div>
                ...
                <div className="grid grid-cols-5 gap-3">
                  {[
                    { label: 'Direction', value: selectedEnhancedAlert.direction, color: selectedEnhancedAlert.direction === 'LONG' ? 'green' : 'red' },
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {[
                      { label: 'Overall', value: selectedEnhancedAlert.scores?.overall || selectedEnhancedAlert.overall_score },
                      { label: 'Technical', value: selectedEnhancedAlert.scores?.technical },
                      { label: 'Fundamental', value: selectedEnhancedAlert.scores?.fundamental },
                      { label: 'Catalyst', value: selectedEnhancedAlert.scores?.catalyst },
                      { label: 'Confidence', value: selectedEnhancedAlert.scores?.confidence },
                    ].map((score, idx) => (
                      <div key={idx} className="bg-zinc-900/50 rounded p-2 text-center">
                        <span className="text-[9px] text-zinc-500 uppercase block">{score.label}</span>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      { label: 'RVOL', value: `${selectedEnhancedAlert.features?.rvol?.toFixed(1)}x` },
                      { label: 'RSI', value: selectedEnhancedAlert.features?.rsi?.toFixed(0) },
                      { label: 'VWAP Dist', value: `${selectedEnhancedAlert.features?.vwap_distance?.toFixed(1)}%` },
                      { label: 'Trend', value: selectedEnhancedAlert.features?.trend },
                    ].map((item, idx) => (
                      <div key={idx} className="bg-zinc-900/50 rounded p-2 text-center">
                        <span className="text-[9px] text-zinc-500 uppercase block">{item.label}</span>
                ...
                {/* Strategy Match */}
                {selectedEnhancedAlert.matched_strategies?.length > 0 && (
                  <div className="p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-purple-400 uppercase font-semibold">Matched Strategies</span>
                      <span className="text-xs text-zinc-400">{selectedEnhancedAlert.matched_strategies_count}/77 rules</span>
                    </div>
                    <div className="flex flex-wrap gap-1">
                      {selectedEnhancedAlert.matched_strategies.slice(0, 5).map((s, idx) => (
