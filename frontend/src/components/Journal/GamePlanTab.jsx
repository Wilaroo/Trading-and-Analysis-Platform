...
const GamePlanTab = () => {
  ...
  const handleRefreshMovers = async () => {
    try {
      ...
      await api.post('/api/dynamic-universe/rebuild', null, { timeout: 60000 });
      await loadData();
    } catch (err) {
      console.error('Failed to refresh dynamic movers:', err);
    } finally {
    ...
    <div className="space-y-4" data-testid="gameplan-tab">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="col-span-2 space-y-3">
          ...
          {/* Dynamic Movers (v19.34.211 — today's IB movers + catalysts) */}
          <div className="p-3 rounded-lg bg-white/5 border border-white/10" data-testid="dynamic-movers-section">
            <button
              onClick={() => setExpandedSection(expandedSection === 'dynamic_movers' ? null : 'dynamic_movers')}
              className="w-full flex items-center justify-between"
              data-testid="dynamic-movers-toggle"
            >
              <div className="flex items-center gap-2">
                <ChevronRight className={`w-4 h-4 text-zinc-500 transition-transform ${expandedSection === 'dynamic_movers' ? 'rotate-90' : ''}`} />
                <Zap className="w-4 h-4 text-amber-400" />
                <span className="text-sm font-medium">Dynamic Movers</span>
                <span className="text-xs text-zinc-500">({gamePlan?.dynamic_movers?.length || 0})</span>
                {gamePlan?.dynamic_movers_regime && (
                  <span className={`text-[11px] px-1.5 py-0.5 rounded ${
                    gamePlan.dynamic_movers_regime === 'CONFIRMED_UP' ? 'bg-emerald-500/20 text-emerald-400'
                    : gamePlan.dynamic_movers_regime === 'CONFIRMED_DOWN' ? 'bg-red-500/20 text-red-400'
                    : 'bg-zinc-500/20 text-zinc-400'
                  }`}>
                    {gamePlan.dynamic_movers_regime?.replace('CONFIRMED_', '')}
                  </span>
                )}
              </div>
              <button
                onClick={(e) => { e.stopPropagation(); handleRefreshMovers(); }}
                disabled={refreshingMovers}
                className="px-2 py-1 rounded bg-amber-500/20 text-amber-400 text-xs hover:bg-amber-500/30 flex items-center gap-1 disabled:opacity-50"
                data-testid="dynamic-movers-refresh"
              >
                <RefreshCw className={`w-3 h-3 ${refreshingMovers ? 'animate-spin' : ''}`} />
                {refreshingMovers ? 'Scanning…' : 'Refresh'}
              </button>
            </button>
            <AnimatePresence>
              {expandedSection === 'dynamic_movers' && (
                <motion.div
                  initial={{ height: 0, opacity: 0 }}
                  animate={{ height: 'auto', opacity: 1 }}
                  exit={{ height: 0, opacity: 0 }}
                ...
                >
                  {(!gamePlan?.dynamic_movers || gamePlan.dynamic_movers.length === 0) ? (
                    <div className="text-center py-4 text-zinc-500 text-xs">
                      No dynamic movers yet. Built each premarket + intraday from IB movers, catalysts & held positions. Hit Refresh to build now.
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-2" data-testid="dynamic-movers-list">
                      {gamePlan.dynamic_movers.map((m, idx) => (
                        <div key={`${m.symbol}-${idx}`} className="p-2 rounded bg-black/30 border border-white/5" data-testid={`dynamic-mover-${m.symbol}`}>
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-bold text-white">{m.symbol}</span>
                            <span className="text-[11px] px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-300 font-mono">{m.score}</span>
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {(m.sources || []).slice(0, 4).map((src, i) => {
                              const kind = src.split(':')[0];
                              const tone = kind === 'held' ? 'bg-blue-500/20 text-blue-300'
                                : kind === 'watchlist' ? 'bg-violet-500/20 text-violet-300'
                                : kind === 'catalyst' ? 'bg-emerald-500/20 text-emerald-300'
                                : kind === 'mover' ? 'bg-amber-500/20 text-amber-300'
                                : 'bg-zinc-600/30 text-zinc-400';
                              return (
                                <span key={i} className={`text-[10px] px-1 py-0.5 rounded ${tone}`}>
                                  {src.replace('mover:', '').replace('catalyst:', '').replace('TOP_PERC_', '').replace('_', ' ').toLowerCase()}
                                </span>
                              );
                            })}
                          </div>
