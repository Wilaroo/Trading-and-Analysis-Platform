            ...
            {loading && suggestions.length === 0 ? (
              ...
              suggestions.slice(0, 5).map((suggestion, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Compact single-line header with priority, title, and dismiss */}
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <span className={`px-1 py-0.5 text-[8px] rounded ${config.bg} ${config.text} font-bold uppercase shrink-0`}>
                          {suggestion.priority}
                        </span>
                        ...
                        {suggestion.symbol && (
                          <button
                            ...
                            className="flex items-center gap-0.5 px-1 py-0.5 bg-cyan-500/20 text-cyan-400 rounded text-[9px] font-medium hover:bg-cyan-500/30 transition-colors shrink-0"
                          >
                            {suggestion.symbol}
                          </button>
                        )}
                      ...
                      <button
                        ...
                        className="p-0.5 hover:bg-zinc-600 rounded opacity-40 hover:opacity-100 transition-opacity shrink-0"
                        title="Dismiss"
                      >
                    ...
                    {/* Compact message - single line with ellipsis */}
                    <p className="text-[10px] text-zinc-400 mt-1 truncate" title={suggestion.message}>
                      {suggestion.message}
                    </p>
                    
                    {/* Compact action hint */}
                    {suggestion.suggested_action && (
                      <div className="text-[9px] text-zinc-500 mt-0.5 truncate italic">
                        → {suggestion.suggested_action}
                      </div>
                    )}
                  </motion.div>
...
const AICommandPanel = ({ 
  ...
  // Scroll to bottom of chat messages (within container only, not page)
  useEffect(() => {
