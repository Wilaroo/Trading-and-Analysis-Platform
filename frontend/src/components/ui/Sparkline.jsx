/**
 * Sparkline - Reusable mini chart component
 * 
 * Used for displaying small trend lines in cards, position displays, etc.
 * Properly contained within its parent element.
 * 
 * Usage:
 *   <div className="h-6 w-16 overflow-hidden rounded">
 *     <Sparkline data={[50, 52, 48, 55]} color="emerald" />
 *   </div>
 */
...
// Pre-configured sparkline with container for quick use
export const SparklineWithContainer = ({ 
  data, 
  pnl, 
  pnlPercent, 
  height = 'h-6', 
  ...
  const chartColor = color || (pnl >= 0 ? 'emerald' : 'rose');
  const chartData = data || generateSparklineData(pnl, pnlPercent);
  
  return (
    <div className={`${height} ${width} overflow-hidden rounded`}>
