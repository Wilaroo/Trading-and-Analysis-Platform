...
const TradeExecutionHealthCard = memo(({ onClick }) => {
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <span className="text-[14px] font-mono text-zinc-500">exec</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <span className={`text-[14px] font-mono font-bold ${s.text}`}>
        EXEC {s.label}
      </span>
      {level !== 'insufficient_data' && (
        <span className={`text-[14px] font-mono ${s.text} opacity-70`}>
          {ratePct}%
        </span>
      )}
    </button>
