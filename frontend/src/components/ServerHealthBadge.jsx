...
const ServerHealthBadge = memo(() => {
  ...
  return (
    ...
    >
      ...
      {pingMs !== null && (
        <span className={`text-[14px] font-mono font-bold ${latencyColor}`}>{pingMs}ms</span>
      )}
      {data?.threads && (
        <span className={`text-[14px] font-mono ${data.threads > 80 ? 'text-yellow-400' : 'text-zinc-500'}`}>{data.threads}T</span>
      )}
      {data?.memory_mb && (
        <span className="text-[14px] font-mono text-zinc-600">{data.memory_mb > 1024 ? `${(data.memory_mb/1024).toFixed(1)}G` : `${Math.round(data.memory_mb)}M`}</span>
      )}
    </div>
  );
});
