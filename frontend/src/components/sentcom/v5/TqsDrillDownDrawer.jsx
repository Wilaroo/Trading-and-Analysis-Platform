...
import { X, TrendingUp, TrendingDown, GitBranch } from 'lucide-react';
...
const TqsDrillDownDrawer = () => {
  ...
  useEffect(() => {
    const onOpen = (e) => {
      const { symbol, source, card: cardObj } = e.detail || {};
      if (!symbol) return;
      setMeta({ symbol, source: source || 'alert' });
      setCard(cardObj || null);
      setWhyOpen(false);
      setOpen(true);
          ...
          <div className="flex items-center gap-3 shrink-0">
            {detail && (
              <button
                ...
                data-testid="why-trace-open"
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-zinc-700 bg-zinc-900 text-[11px] uppercase tracking-wider text-zinc-300 hover:text-white hover:border-zinc-500 transition-colors"
              >
