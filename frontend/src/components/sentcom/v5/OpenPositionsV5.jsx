...
// v404 (2026-06-23) — operator transparency: lifecycle timestamps
// (alert -> entry -> refreshed -> exit) on each position so backlogged /
// stale flushes are visible at a glance. ET wall-clock + relative age.
const fmtET = (ts) => {
  if (!ts) return null;
  const d = new Date(ts);
...
const TimeCell = ({ label, ts, tone }) => {
  const clock = fmtET(ts);
  const age = relAge(ts);
  return (
    <div data-testid={`time-cell-${String(label).toLowerCase()}`}>
      <div className="text-[13px] uppercase tracking-wider text-zinc-600">{label}</div>
      <div className={tone || 'text-zinc-300'}>
        {clock
          ? <>{clock}<span className="text-zinc-600"> ET</span>{age ? <span className="text-zinc-600"> · {age}</span> : null}</>
          : '—'}
      </div>
    </div>
  );
    ...
    >
      ...
      {expanded && (
        ...
        >
          ...
          {/* v404 — lifecycle timestamps (alert -> entry -> refreshed -> exit) */}
          <div
            className="grid grid-cols-4 gap-1 v5-mono text-[12px]"
            data-testid={`open-position-times-${position.symbol}`}
          >
            <TimeCell label="Alert" ts={position.alert_time || position.triggered_at || position.created_at} />
            <TimeCell label="Entry" ts={position.entry_time || position.executed_at || position.opened_at} />
            <TimeCell label="Refreshed" ts={position.last_update || position.updated_at} tone="text-cyan-300" />
            <TimeCell label="Exit" ts={position.exit_time || position.closed_at} tone="text-zinc-400" />
          </div>
