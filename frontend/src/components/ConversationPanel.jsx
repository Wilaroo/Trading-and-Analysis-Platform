...
const ConversationPanel = ({ 
  ...
  return (
    <div className={`flex flex-col h-full bg-[#0F1419] ${className}`} data-testid="conversation-panel">
      {/* Header */}
        ...
        {chatMessages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-xs text-zinc-500 mt-1">Ask questions, get trade ideas, or discuss strategy</p>
            </div>
          </div>
        ) : (
          <>
      ...
      <div className="p-3 border-t border-white/10 space-y-2">
        ...
        <form onSubmit={handleSubmit} className="relative">
          <input
            ...
            disabled={loading}
            className="w-full bg-white/5 border border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 transition-all disabled:opacity-50"
            data-testid="conversation-input"
          />
          <button
            type="submit"
            disabled={!inputValue.trim() || loading}
            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg bg-gradient-to-r from-cyan-500 to-cyan-600 text-white shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
